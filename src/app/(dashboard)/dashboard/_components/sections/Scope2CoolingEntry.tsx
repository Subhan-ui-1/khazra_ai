...
const Scope2CoolingEntry: React.FC = () => {
  ...
  const handleSubmit = () => {
    const newRecord = {
      id: editingItem ? editingItem.id : `cooling_${Date.now()}`,
      ...formData,
      consumption: parseFloat(formData.consumption),
      calculatedEmissions: parseFloat(formData.consumption) * 0.0006 // Example factor
    };
    if (editingItem) {
      setCoolingData(prev => prev.map(item => item.id === editingItem.id ? newRecord : item));
    } else {
      setCoolingData(prev => [...prev, newRecord]);
    }
    setShowForm(false);
    setEditingItem(null);
    setFormData({
  ...
  const filteredData = selectedFacility === 'all'
    ? coolingData
    : coolingData.filter(item => item.facilityId === selectedFacility);
        ...
        <div className="bg-white border border-cyan-200 rounded-lg p-6">
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Cooling Type</label>
                <input
                  type="text"
                  value={formData.coolingType}
                  onChange={(e) => setFormData((prev: any) => ({ ...prev, coolingType: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-cyan-500 focus:border-cyan-500"
                  placeholder="e.g., Chiller, AC, etc."
                />
              </div>
              <div>
                ...
                <tr key={record.id}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => setCoolingData(prev => prev.filter(item => item.id !== record.id))}
