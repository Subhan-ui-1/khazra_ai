...
const Scope2SteamEntry: React.FC = () => {
  ...
  const handleSubmit = () => {
    const newRecord = {
      id: editingItem ? editingItem.id : `steam_${Date.now()}`,
      ...formData,
      consumption: parseFloat(formData.consumption),
      calculatedEmissions: parseFloat(formData.consumption) * 0.0005 // Example factor
    };
    if (editingItem) {
      setSteamData(prev => prev.map(item => item.id === editingItem.id ? newRecord : item));
    } else {
      setSteamData(prev => [...prev, newRecord]);
    }
    setShowForm(false);
    setEditingItem(null);
    setFormData({
  ...
  const filteredData = selectedFacility === 'all'
    ? steamData
    : steamData.filter(item => item.facilityId === selectedFacility);
                ...
                <tr key={record.id}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => setSteamData(prev => prev.filter(item => item.id !== record.id))}
