...
export default function Scope2Section() {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    emissionByVehicle: 0,
    recentActivities: [],
    purchasedElectricityEmissions: 0,
    purchasedSteamEmissions: 0,
    purchasedHeatEmissions: 0,
    purchasedCoolingEmissions: 0,
  });
  ...
  const scope2DataByDuration: Scope2Data = {
    "This Year": [
      {
        ...
        value: dashboardData.purchasedElectricityEmissions, // Assuming 80% is electricity
        percentage: Number(dashboardData.totalEmissions > 0 ? (dashboardData.purchasedElectricityEmissions / dashboardData.totalEmissions * 100).toFixed(2) : 0),
        color: "fill-[#6f33e8]",
        rawColor: "#6f33e8",
      },
      {
        label: "steam",
        value: dashboardData.purchasedSteamEmissions, // Assuming 15% is steam
        percentage: Number(dashboardData.totalEmissions > 0 ? (dashboardData.purchasedSteamEmissions / dashboardData.totalEmissions * 100).toFixed(2) : 0),
        color: "fill-[#00bbff]",
        rawColor: "#00bbff",
      },
      {
        label: "Heating & Cooling",
        value: dashboardData.purchasedHeatEmissions + dashboardData.purchasedCoolingEmissions, // Assuming 5% is heating/cooling
        percentage: Number(dashboardData.totalEmissions > 0 ? ((dashboardData.purchasedHeatEmissions + dashboardData.purchasedCoolingEmissions) / dashboardData.totalEmissions * 100).toFixed(2) : 0),
        color: "fill-[#ff8c09]",
        rawColor: "#ff8c09",
      },
    ],
    "Last Year": [
      {
        label: "Purchased Electricity",
        value: 0, // 10% higher than current
        percentage: 0,
        color: "fill-[#6f33e8]",
        rawColor: "#6f33e8",
      },
    ...
    Comparison: [
      {
        label: "This Year",
        value: dashboardData.scope2Emissions,
        percentage: 100,
        color: "fill-[#6f33e8]",
        rawColor: "#6f33e8",
      },
      ...
      // {
      //   label: "Heating & Cooling",
      //   value: dashboardData.purchasedHeatEmissions + dashboardData.purchasedCoolingEmissions,
      //   percentage: Number(dashboardData.totalEmissions > 0 ? ((dashboardData.purchasedHeatEmissions + dashboardData.purchasedCoolingEmissions) / dashboardData.totalEmissions * 100).toFixed(2) : 0),
      //   color: "fill-[#ff8c09]",
      //   rawColor: "#ff8c09",
      // },
  ...
  const energyEfficiencyData = [
    ...
    { month: "Jun", efficiency: 0.19, target: 0.15 },
  ...
  const pieData = {
    labels: scope2Sources.map((item) => item.label),
    datasets: [
      {
  ...
  const unitOptions = ["MWh", "kWh", "GJ", "MJ", "BTU"];
  ...
  const findMax = (data: any) => {
    ...
    if (max === purchasedElectricityEmissions) {
      return {
        ...
        value: purchasedElectricityEmissions,
        icon: '🔌'
      };
  ...
  const maximum = findMax(dashboardData);
  ...
  const scope2BreakdownData = [
    {
      ...
      name: "Purchased Electricity",
      value: dashboardData?.purchasedElectricityEmissions?.toFixed(1),
      percentage:
        dashboardData?.totalEmissions > 0
          ? (
              ((dashboardData?.purchasedElectricityEmissions) /
                dashboardData?.scope2Emissions) *
              100
            ).toFixed(2)
          : 0,
      color: "#0f5744",
    ...
    },
    {
      id: "steam",
      name: "Steam",
      value: dashboardData?.purchasedSteamEmissions?.toFixed(1),
      percentage:
        dashboardData?.totalEmissions > 0
          ? (
              ((dashboardData?.purchasedSteamEmissions) /
                dashboardData?.scope2Emissions) *
              100
            ).toFixed(2)
          : 0,
    ...
    },
    {
      ...
      value: (dashboardData?.purchasedHeatEmissions + dashboardData?.purchasedCoolingEmissions)?.toFixed(1),
      percentage:
        dashboardData.totalEmissions > 0
          ? (
              ((dashboardData?.purchasedHeatEmissions + dashboardData?.purchasedCoolingEmissions) /
                dashboardData?.scope2Emissions) *
              100
            ).toFixed(2)
          : 0,
  ...
  const metricsData = [
    {
      ...
      subtitle: `tonnes CO₂e • ${
        dashboardData.totalEmissions > 0
          ? (
              (dashboardData.scope2Emissions / dashboardData.totalEmissions) *
              100
            ).toFixed(1)
          : 0
      }% of total emissions`,
      ...
      progress:
        dashboardData.totalEmissions > 0
          ? (dashboardData.scope2Emissions / dashboardData.totalEmissions) * 100
          : 0,
    },
    {
      id: "purchased-electricity",
      title: maximum.name,
      value: maximum.value.toFixed(1),
      // change: '▼ 12.3%',
      // changeType: 'decrease',
    ...
    <div className="space-y-10">
      ...
      <div className="border-b border-green-100 pb-6">
        ...
        <p className="text-black opacity-70 max-w-4xl leading-relaxed">
          ...
          electricity, steam, heating, and cooling consumed by the organization,
          including detailed analysis of energy sources and efficiency metrics.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metricsData.map((metric) => (
          <div
            key={metric.id}
            className="bg-white border border-green-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1"
          >
      ...
      <div className="w-full grid grid-cols-3 gap-4">
        <div className="xl:w-[100%] bg-white col-span-2 border border-green-100 rounded-xl p-6 shadow-sm flex flex-col justify-between">
        ...
        {scope2BreakdownData.map((source) => (
          <div
            key={source.id}
            className="bg-white border border-green-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 w-full"
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-green-50 rounded-lg flex items-center justify-center text-xl">
                {source.id === "purchased-electricity"
                  ? "🔌"
                  : source.id === "steam"
                  ? "💨"
                  : "❄️"}
            ...
            <div className="text-xs text-black opacity-70">
              {source.id === "purchased-electricity"
                ? "12 meters • Grid mix, renewable certificates"
                : source.id === "steam"
                ? "8 steam lines • Natural gas, biomass"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, energyType: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                    required
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, gridLocation: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        customEmissionFactor: e.target.value,
                      })
                    }
                    className="w-full px-3 py-2 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-green-100 text-sm">
                      {reviewMode === "add"
                        ? "New Purchased Electricity Entry"
                        : "Edit Purchased Electricity Entry"}
                    </p>
                  </div>
                </div>
                <button
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between py-2">
                        ...
                        <span className="text-gray-900 font-semibold">
                          {reviewData.amountOfConsumption}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between py-2 border-b border-gray-100">
                        ...
                        <span className="text-gray-900 font-semibold">
                          {reviewData.customEmissionFactor || "Not specified"}
                        </span>
                      </div>
