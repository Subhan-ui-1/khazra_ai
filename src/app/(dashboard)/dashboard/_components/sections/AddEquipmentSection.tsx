...
const installationYears = Array.from(
  { length: 30 },
...
const AddEquipmentSection = ({ onComplete }: AddEquipmentSectionProps) => {
  ...
  const fetchEquipments = async () => {
    try {
      ...
      const response = await getRequest(
        `equipments/getEquipments?${queryParams}`,
  ...
  const fetchEquipmentTypes = async () => {
    try {
      const response = await getRequest(
        "equipment-types/getEquipmentTypes",
        tokenData.accessToken
      );
      if (response.success) {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedEquipments(
        equipmentData.map((equipment) => equipment.id || equipment._id)
      );
    } else {
      setSelectedEquipments([]);
    }
  ...
  const handleBulkDelete = async () => {
    if (
      !confirm(
        `Are you sure you want to delete ${selectedEquipments.length} selected equipments?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success(
        `${selectedEquipments.length} equipments deleted successfully`
      );
      setSelectedEquipments([]);
      fetchEquipments();
    } catch (error: any) {
  ...
  const handleBulkStatusUpdate = async (newStatus: string) => {
    try {
      ...
      toast.success(
        `${selectedEquipments.length} equipments status updated successfully`
      );
      setSelectedEquipments([]);
      fetchEquipments();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Bulk Actions */}
      {(onComplete ? null : selectedEquipments.length > 0) && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-blue-900">
                {selectedEquipments.length} equipment
                {selectedEquipments.length > 1 ? "s" : ""} selected
              </span>
            </div>
            <div className="flex gap-2">
                ...
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                    <input
                      type="checkbox"
                      checked={
                        selectedEquipments.length === equipmentData.length &&
                ...
                {loading ? (
                  ...
                  <tr>
                    <td
                      ...
                      className="px-6 py-8 text-center text-gray-500"
                    >
                      No equipments found. Add your first equipment to get
                      started.
                    </td>
                  </tr>
                ) : (
                  equipmentData.map((equipment, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getEquipmentTypeColor(
                            equipment.equipmentType?.equipmentName ||
                              equipment.equipmentTypeId
                          )}`}
                        >
                          {equipment.equipmentTypeId?.equipmentName ||
